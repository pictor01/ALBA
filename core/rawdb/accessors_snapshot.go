...
func ReadSnapshotDisabled(db albadb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db albadb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db albadb.KeyValueWriter) {
...
func ReadSnapshotRoot(db albadb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db albadb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db albadb.KeyValueWriter) {
...
func ReadAccountSnapshot(db albadb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db albadb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db alba.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db albadb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db albadb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db albadb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db albadb.Iteratee, accountHash common.Hash) albadb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db albadb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db albadb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db albadb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db albadb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db albadb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db albadb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db albadb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db albadb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db albadb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db albadb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db albadb.KeyValueWriter, status []byte) {
